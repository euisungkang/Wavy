...
module.exports = {
    ...
    async execute(interaction) {
        ...
        await interaction.reply({  content: "Check your DMs <@" + interaction.user.id + ">\n" + 
                                        "Your editable <#820051777650556990> features were sent"  })
...
async function editCommand(interaction) {
    ...
    let initialMSG = await interaction.user.send({ embeds: [embed] })

    embed.spliceFields(0, 1)

    let subscriptions = await database.getAllSubscriptions(interaction.user.id)
    let rolePurchased
    ...
    if (subscriptions.size == 0) {
        ...
        await subscriptions.forEach((value, key) => {
            if (key >= 0 && key <= 3) {
                ...
                embed.addFields({ name: "\u200B\nServer Name: " + value[interaction.user.id].newName + " 💎", value: "Expiration: " + value[interaction.user.id].date.toDate().toLocaleDateString()})
                initialMSG.react("💎")
            }
        })
        initialMSG.react('❌')
    ...
    if (reaction.size < 1) {
        await interaction.user.send({ content: "Request Timed Out. Try requesting a new /edit, and try again. *Boomer Fingers*" })
        return false
    }
    ...
    if (reactionName == '❌') {
        await interaction.user.send({ content: "Your /edit request is cancelled.\nFeel free to type /edit in any text channel if you change your mind"})
        return false
        ...
        let roleOBJ = await interaction.guild.roles.fetch(role.id).catch(err => console.log(err))
        ...
        let featureMSG = await interaction.user.send({ embeds: [embed2] })
        ...
        if (reactionName == 'shek') {
            optionMSG = await interaction.user.send({ content: "Current role name is: " + role.name + "\nWhat do you want to change the name to?"})

            let newName = await market.awaitResponse(optionMSG.channel, filter, 30000, true)
            if (newName == false) {
            await interaction.user.send({ content: "Request timed out. Try sending a new /edit request next time "})
            return false
            }
            ...
            await database.updateRoles(interaction.user.id, role, role.tier)

            await interaction.user.send({ content: "Successfully edited the name of your custom role (Tier " + role.tier + ") to " + role.name})

        } else if (reactionName == 'srsly') {
            optionMSG = await interaction.user.send({ content: "Current role hexcode color: " + role.color + "\nWhat do you want to change the color to? Enter a valid hexcode." +
            ...
            await interaction.user.send({ content: "Successfully edited the color of your custom role (Tier " + role.tier + ") to " + role.color})

        } else if (reactionName == 'PikaO') {

            let optionMSG = await interaction.user.send({ content: "Upload any icon for your custom role.\nAccepted dimensions **must** be of **1:1** aspect ratio and **256KB** max size\n*Since it's an icon, I suggest you upload a transparent PNG. Your call though*" +
            ...
            return await processIcon(interaction.user, optionMSG.channel, roleOBJ)

        } else if (reactionName == 'habey') {
            let positions = await database.getRolePositions();

            let tier = role.tier
            let target = await interaction.guild.members.fetch(interaction.user.id, { force: true })

            if (tier == 4 && target.roles.cache.has(positions[4])) {
            await interaction.user.send({ content: "You are already fo rank 𝓡𝓸𝔂𝓪𝓵𝓽𝔂, and have the highest custom role Tier" })
            return false
            }else if (tier == 1 && !target.roles.cache.has(positions[1])) {
            await interaction.user.send({ content: "You have to be of at least 𝕒 𝕖 𝕤 𝕥 𝕙 𝕖 𝕥 𝕚 𝕔 rank to upgrade to a **Tier 1 Custom Role**" })
            return false
            } else if (tier == 2 && !target.roles.cache.has(positions[2])) {
            await interaction.user.send({ content: "You have to be of at least 𝒢𝓇❀❁𝓋𝓎 rank to upgrade to a **Tier 2 Custom Role**" })
            return false
            } else if (tier == 3 && !target.roles.cache.has(positions[3])) {
            await interaction.user.send({ content: "You have to be of at least 𝓦𝓪𝓿𝔂 rank to upgrade a **Tier 3 Custom Role**" })
            return false
            } else if (tier == 4 && !target.roles.cache.has(positions[4])) {
            await interaction.user.send({ content: "You have to be of 𝓡𝓸𝔂𝓪𝓵𝓽𝔂 rank to upgrade to a **TIer 4 Custom Role**" })
            return false
            }
            ...
            let wallet = await database.getCum(interaction.user.id)

            if (priceDifference > wallet) {
            interaction.user.send({ content: "You currently have " + wallet + "<:HentaiCoin:814968693981184030>\n" +
            ...
            optionMSG = await interaction.user.send({ content: "An upgrade from Tier **" + tier + "** to Tier **" + (tier + 1) + 
            ...
            if (emoji == '❌') {
            await interaction.user.send({ content: "Got it, your tier upgrade won't go through" })
            return false
            } else if (emoji == '✅') {

            let pivotOBJ = await interaction.guild.roles.fetch(positions[tier + 1]).catch(err => console.log(err))
            ...
            await database.editRole(interaction.user.id, role)

            database.removeCum(interaction.user, priceDifference)

            await interaction.user.send({ content: "Successfully upgraded your " + role.name + " role from Tier " + tier + " to Tier " + (tier + 1) + 
        ...
        if (badges.length > 1) {
            ...
            featureMSG = await interaction.user.send({ embeds: [embed2] })

            filter = (m) => m.author.id == interaction.user.id
            badgeID = await market.awaitResponse(featureMSG.channel, filter, 30000, false)
            if (await validInput(interaction.user, badgeID, badges.length) == false)
                return false
            badgeID = badgeID - 1
        ...
        let badgeOBJ = await interaction.guild.roles.fetch(badges[badgeID].id).catch(err => console.log(err))
        ...
        let optionMSG
        if (reactionName == 'shek') {
            optionMSG = await interaction.user.send({ content: "Current badge name is: " + badge.name + "\nWhat do you want to change the name to?"})
            ...
            await database.editBadges(interaction.user.id, badges)

            await interaction.user.send({ content: "Successfully edited the name of your custom badge to " + badge.name})

        } else if (reactionName == 'srsly') {
            optionMSG = await interaction.user.send({ content: "Current badge hexcode color: " + badge.color + "\nWhat do you want to change the color to? Enter a valid hexcode." +
                                                            "\n\n*Use this online color picker to get your desired hex code:* <https://htmlcolorcodes.com/color-picker/>" })
            ...
            await interaction.user.send({ content: "Successfully edited the color of your custom badge to " + badge.color})

        } else if (reactionName == 'PikaO') {
            
            let optionMSG = await interaction.user.send({ content: "Upload any custom icon for your badge.\nAccepted dimensions **must** be of **1:1** and **256KB** max size\n*Since it's an icon, I suggest you upload a transparent PNG. Your call though*" +
                                                                "\n\nYou can use whatever resize/compress/transparent tool, but here's some quick links\nPNG Transparent(izer)?: <https://onlinepngtools.com/create-transparent-png>\nImage Resizer: <https://imageresizer.com/>\nImage Compressor: <https://imagecompressor.com>\nImage Cropper: <https://www.iloveimg.com/crop-image>" })

            return await processIcon(interaction.user, optionMSG.channel, badgeOBJ)
        }
        ...
        if (nicknames.length > 1) {
            ...
            if (await validInput(interaction.user, nicknameID, nicknames.length) == false)
                return false
        ...
        let nicknameOBJ = await interaction.guild.members.fetch(nickname.id, { force: true })

        filter = (m) => m.author.id == interaction.user.id
        let optionMSG = await interaction.user.send({ content: "User: " + nickname.username + " has a restricted nickname of " + nickname.newNickname + "\nWhat do you want to change the nickname to?" })
        let newName = await market.awaitResponse(optionMSG.channel, filter, 30000, true)
        if (newName == false) {
            await interaction.user.send({ content: "Request timed out. Try sending a new /edit request next time "})
            return false
        ...
        format[interaction.user.id] = nicknames
        await database.updateRestrictedNicknames(format)

        await interaction.user.send({ content: "Successfully edited the nickname of " + nickname.username + " to " + nickname.newNickname})
        ...
        let serverIcon = serverIconFormat[interaction.user.id]
        ...
        let featureMSG = await interaction.user.send({ content: "Upload the new server icon as an image.\nAccepted dimensions **must** be of **1:1** and **8MB** max size" +
        ...
        if (aspectRatio != "1:1") {
            await interaction.user.send({ content: "The server icon dimensions **must** be of **1:1** aspect ratio\n" + 
            ...
            await interaction.user.send({ content: "The server icon size **must** be **less than 8MB**" +
        ...
        serverIconFormat[interaction.user.id] = serverIcon

        await database.updateRestrictedServerIcon(serverIconFormat)
        await interaction.guild.setIcon(newIcon.url)

        await interaction.user.send({ content: "Successfully edited the server icon. Changes will be actualized shortly" +
        ...
        let serverName = serverNameFormat[interaction.user.id]
        ...
        let featureMSG = await interaction.user.send({ embeds: [embed2] })
        filter = (m) => m.author.id == interaction.user.id
        let optionMSG = await interaction.user.send({ content: "What would you like to change the server name to?"})
        let newServerName = await market.awaitResponse(optionMSG.channel, filter, 30000, true)
        if (newServerName == false) {
            await interaction.user.send({ content: "Request Timed Out or Invalid Input. Try sending a new /edit request next time"})
            return false
        }
        ...
        serverNameFormat[interaction.user.id] = serverName

        interaction.guild.setName(newServerName)

        await database.updateRestrictedServerName(serverNameFormat)

        await interaction.user.send({ content: "Successfully edited server name from **" + oldName + "** to **" + newServerName + "**"})
    }
}
